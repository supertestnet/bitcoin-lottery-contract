...
- Detect unexpected delays and warn users about them
  - If the funding transaction was already broadcasted, there is a possible cheating attempt. The user might need to wait up to 16 blocks but they are guaranteed to either win or lose fairly if they wait that long
  - If the funding transaction was not yet broadcasted, there is a possible griefing attempt. The user should probably abort
